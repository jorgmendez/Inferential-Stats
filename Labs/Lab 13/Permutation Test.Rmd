...
$$
where 𝜇is the population mean birth weight (in grams).

We’ll use resampling (permutation) methods instead of traditional parametric tests to approximate the null distribution of the mean difference.
...
If that’s true, then the labels “Yes” and “No” (for Smoker) are meaningless — we could shuffle them randomly, and any difference in means would just be due to chance.

So, we’ll simulate that process many times:

1.- Randomly shuffle the “Smoker” labels.

2.- Recalculate the mean difference each time.

3.- Store those differences to create a null distribution.

This gives us a picture of what differences we’d expect if the null hypothesis were true.
...
Now that we have the null distribution (the 5,000 simulated mean differences), we can check how extreme our observed statistic (≈215 g) is compared to that distribution.
...
So, we’ll calculate the proportion of simulated differences that are greater than or equal to our observed difference.
...
It doesn’t mean the probability is exactly zero — it just means the p-value is smaller than the resolution of our simulation (1 / 5000 = 0.0002).
...
In this lab, we tested whether babies born to non-smoking mothers weigh more on average than those born to smoking mothers using a permutation test with 5,000 random shuffles of the “Smoker” labels.
...
Therefore, we reject H₀ and conclude that babies of non-smoking mothers weigh significantly more on average than those of smoking mothers.
